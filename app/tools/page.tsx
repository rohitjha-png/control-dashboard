...
import { Wrench, ChevronRight, Bot } from "lucide-react";
...
export default function ToolsPage() {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-auto">
      <Topbar title="Tool Calls" subtitle="Inspect tool calls per agent session" />
      <div className="p-6">
        <p className="text-sm text-gray-400 mb-5">
          Select an agent to browse its tool definitions, or view tool calls on the{" "}
          <Link href="/calls" className="text-blue-400 hover:underline">call detail page</Link>.
        </p>
        {isLoading && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {agents.map((agent) => (
            <Link key={agent.id} href={`/agents/${agent.id}?tab=tools`}>
              <Card className="hover:border-gray-600 transition-all cursor-pointer h-full">
                <CardContent className="p-5 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">View tools</p>
                  </div>
